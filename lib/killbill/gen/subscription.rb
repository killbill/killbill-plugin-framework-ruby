    ...
    module Model
      ...
      class Subscription
        ...
        def to_java()
          ...
          state = "Java::com.ning.billing.entitlement.api.user.SubscriptionState::#{state.to_s}" if !state.nil?

          # conversion for source_type [type = com.ning.billing.entitlement.api.user.SubscriptionSourceType]
          source_type = "Java::com.ning.billing.entitlement.api.user.SubscriptionSourceType::#{source_type.to_s}" if !source_type.nil?
          ...
          current_plan = current_plan.to_java if !current_plan.nil?

          # conversion for last_active_plan [type = com.ning.billing.catalog.api.Plan]
          last_active_plan = last_active_plan.to_java if !last_active_plan.nil?

          # conversion for current_price_list [type = com.ning.billing.catalog.api.PriceList]
          current_price_list = current_price_list.to_java if !current_price_list.nil?

          # conversion for current_phase [type = com.ning.billing.catalog.api.PlanPhase]
          current_phase = current_phase.to_java if !current_phase.nil?

          # conversion for last_active_product_name [type = java.lang.String]
          last_active_product_name = last_active_product_name.to_s if !last_active_product_name.nil?

          # conversion for last_active_price_list_name [type = java.lang.String]
          last_active_price_list_name = last_active_price_list_name.to_s if !last_active_price_list_name.nil?

          # conversion for last_active_category_name [type = java.lang.String]
          last_active_category_name = last_active_category_name.to_s if !last_active_category_name.nil?

          # conversion for last_active_billing_period [type = java.lang.String]
          last_active_billing_period = last_active_billing_period.to_s if !last_active_billing_period.nil?
          ...
          pending_transition = pending_transition.to_java if !pending_transition.nil?

          # conversion for previous_transition [type = com.ning.billing.entitlement.api.user.SubscriptionTransition]
          previous_transition = previous_transition.to_java if !previous_transition.nil?
        ...
        def self.to_ruby(j_obj)
          ...
          source_type = source_type.to_s if !source_type.nil?
          ...
          current_plan = Killbill::Plugin::Model::Plan.to_ruby(current_plan) if !current_plan.nil?

          # conversion for last_active_plan [type = com.ning.billing.catalog.api.Plan]
          last_active_plan = j_obj.last_active_plan
          last_active_plan = Killbill::Plugin::Model::Plan.to_ruby(last_active_plan) if !last_active_plan.nil?

          # conversion for current_price_list [type = com.ning.billing.catalog.api.PriceList]
          current_price_list = j_obj.current_price_list
          current_price_list = Killbill::Plugin::Model::PriceList.to_ruby(current_price_list) if !current_price_list.nil?

          # conversion for current_phase [type = com.ning.billing.catalog.api.PlanPhase]
          current_phase = j_obj.current_phase
          current_phase = Killbill::Plugin::Model::PlanPhase.to_ruby(current_phase) if !current_phase.nil?
          ...
          pending_transition = Killbill::Plugin::Model::SubscriptionTransition.to_ruby(pending_transition) if !pending_transition.nil?

          # conversion for previous_transition [type = com.ning.billing.entitlement.api.user.SubscriptionTransition]
          previous_transition = j_obj.previous_transition
          previous_transition = Killbill::Plugin::Model::SubscriptionTransition.to_ruby(previous_transition) if !previous_transition.nil?
          ...
          all_transitions.each do |m|
            # conversion for m [type = com.ning.billing.entitlement.api.user.SubscriptionTransition]
            m = j_obj.m
            m = Killbill::Plugin::Model::SubscriptionTransition.to_ruby(m) if !m.nil?
            tmp << m
          end
          all_transitions = tmp
        end
