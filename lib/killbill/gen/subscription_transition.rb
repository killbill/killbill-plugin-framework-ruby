    ...
    module Model
      ...
      class SubscriptionTransition
        ...
        def to_java()
          ...
          previous_state = "Java::com.ning.billing.entitlement.api.user.SubscriptionState::#{previous_state.to_s}" if !previous_state.nil?

          # conversion for next_state [type = com.ning.billing.entitlement.api.user.SubscriptionState]
          next_state = "Java::com.ning.billing.entitlement.api.user.SubscriptionState::#{next_state.to_s}" if !next_state.nil?

          # conversion for previous_event_id [type = java.util.UUID]
          previous_event_id = java.util.UUID.fromString(previous_event_id.to_s) if !previous_event_id.nil?
          ...
          previous_plan = previous_plan.to_java if !previous_plan.nil?

          # conversion for next_plan [type = com.ning.billing.catalog.api.Plan]
          next_plan = next_plan.to_java if !next_plan.nil?

          # conversion for previous_phase [type = com.ning.billing.catalog.api.PlanPhase]
          previous_phase = previous_phase.to_java if !previous_phase.nil?

          # conversion for next_event_id [type = java.util.UUID]
          next_event_id = java.util.UUID.fromString(next_event_id.to_s) if !next_event_id.nil?
          ...
          next_phase = next_phase.to_java if !next_phase.nil?

          # conversion for previous_price_list [type = com.ning.billing.catalog.api.PriceList]
          previous_price_list = previous_price_list.to_java if !previous_price_list.nil?

          # conversion for next_price_list [type = com.ning.billing.catalog.api.PriceList]
          next_price_list = next_price_list.to_java if !next_price_list.nil?
          ...
          transition_type = "Java::com.ning.billing.entitlement.api.SubscriptionTransitionType::#{transition_type.to_s}" if !transition_type.nil?
        end
        ...
        def self.to_ruby(j_obj)
          ...
          previous_state = previous_state.to_s if !previous_state.nil?

          # conversion for next_state [type = com.ning.billing.entitlement.api.user.SubscriptionState]
          next_state = j_obj.next_state
          next_state = next_state.to_s if !next_state.nil?

          # conversion for previous_event_id [type = java.util.UUID]
          previous_event_id = j_obj.previous_event_id
          previous_event_id = previous_event_id.nil? ? nil : uuid.to_s
          ...
          previous_plan = Killbill::Plugin::Model::Plan.to_ruby(previous_plan) if !previous_plan.nil?

          # conversion for next_plan [type = com.ning.billing.catalog.api.Plan]
          next_plan = j_obj.next_plan
          next_plan = Killbill::Plugin::Model::Plan.to_ruby(next_plan) if !next_plan.nil?

          # conversion for previous_phase [type = com.ning.billing.catalog.api.PlanPhase]
          previous_phase = j_obj.previous_phase
          previous_phase = Killbill::Plugin::Model::PlanPhase.to_ruby(previous_phase) if !previous_phase.nil?

          # conversion for next_event_id [type = java.util.UUID]
          next_event_id = j_obj.next_event_id
          next_event_id = next_event_id.nil? ? nil : uuid.to_s
          ...
          next_phase = Killbill::Plugin::Model::PlanPhase.to_ruby(next_phase) if !next_phase.nil?

          # conversion for previous_price_list [type = com.ning.billing.catalog.api.PriceList]
          previous_price_list = j_obj.previous_price_list
          previous_price_list = Killbill::Plugin::Model::PriceList.to_ruby(previous_price_list) if !previous_price_list.nil?

          # conversion for next_price_list [type = com.ning.billing.catalog.api.PriceList]
          next_price_list = j_obj.next_price_list
          next_price_list = Killbill::Plugin::Model::PriceList.to_ruby(next_price_list) if !next_price_list.nil?
          ...
          transition_type = transition_type.to_s if !transition_type.nil?
        end
